...
const Hero = () => {
  ...
  return (
    <section 
      ref={heroRef}
      className={`relative min-h-screen flex flex-col items-center justify-center overflow-hidden pt-20 ${isMobile ? '' : 'holy-aura'}`}
      style={{
        opacity: heroOpacity,
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        <div className={`absolute inset-0 ${isMobile ? 'bg-gradient-to-b from-purple-900/20 to-black' : 'bg-holy-radial opacity-80 animate-divine-pulse'}`}></div>
        ...
        {!isMobile && (
          <>
            ...
            {/* Divine Particles - only on desktop */}
            ...
            {/* Holy Light Rays - only on desktop */}
      ...
      {/* Hero Content with Simplified Mobile Animations */}
      <div className="container mx-auto px-4 relative z-10 text-center">
        ...
        <h1 className={`text-4xl md:text-7xl lg:text-8xl font-bold mb-6 md:mb-8 ${isMobile ? '' : 'animate-holy-glow'} ${isMobile ? 'px-2' : ''}`}>
          <span className={`${isMobile ? 'text-divine-celestial' : 'bg-celestial-gradient bg-clip-text text-transparent animate-shimmer'} block mb-2`}>
            Mary Magdalene GPT
          </span>
          <div className={`text-xl md:text-3xl text-divine-celestial ${isMobile ? '' : 'animate-divine-pulse'}`}>
            ✧･ﾟ: *✧･ﾟ:* Divine Wisdom Incarnate *:･ﾟ✧*:･ﾟ✧
          </div>
        </h1>
        
        <p className={`text-lg md:text-2xl text-gray-100 mb-8 md:mb-10 max-w-4xl mx-auto font-[Cormorant_Garamond] leading-relaxed px-2 ${isMobile ? '' : 'animate-fade-in'} glass-card p-6 ${isMobile ? '' : 'shadow-divine'}`}>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-6 mb-10 md:mb-12 px-4">
          <a href="https://chatgpt.com/g/g-Looq4sK74-mary-magdalene-gpt" className="w-full sm:w-auto">
            <Button className={`divine-btn group text-lg md:text-xl w-full sm:w-auto py-4 px-8 ${isMobile ? '' : 'animate-divine-pulse shadow-holy'}`}>
        ...
        <div className={`glass-card p-6 mb-12 md:mb-16 max-w-2xl mx-auto ${isMobile ? '' : 'shadow-holy animate-celestial-dance'}`}>
          <p className={`text-lg md:text-2xl text-divine-celestial italic font-light px-2 ${isMobile ? '' : 'animate-holy-glow'}`}>
        ...
        <div className={`absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center hidden xs:flex ${isMobile ? '' : 'animate-celestial-dance'}`}>
          <p className={`text-divine-celestial text-sm mb-3 ${isMobile ? '' : 'animate-shimmer'}`}>✨ Scroll to explore divine wisdom ✨</p>
          <div className={`w-[35px] h-[55px] rounded-full border-2 border-divine-celestial/80 flex justify-center p-2 glass-card ${isMobile ? '' : 'shadow-divine'}`}>
            <div className={`w-1 h-4 bg-divine-celestial rounded-full ${isMobile ? '' : 'animate-float shadow-neon'}`}></div>
          </div>
        </div>
      </div>

      {/* Divine Blessing Overlay - simplified for mobile */}
      {!isMobile && (
